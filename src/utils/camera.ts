...
export function updateCamera() {
  if (store.camera.panning) {
    store.camera.position.x = lerp(
      store.camera.position.x,
      store.camera.targetPosition.x,
      0.05,
    );
    store.camera.position.y = lerp(
      store.camera.position.y,
      store.camera.targetPosition.y,
      0.05,
    );
  }
